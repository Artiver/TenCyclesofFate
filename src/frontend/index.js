...
function showView(viewId) {
    document.querySelectorAll('.view').forEach(v => {
        ...
        // Restore hidden state for game-view: hidden when not active
        if (v.id === 'game-view') {
            v.hidden = true;
        }
    });
    ...
    // Remove hidden attribute so CSS grid can display it
    if (viewId === 'game-view') {
        target.hidden = false;
    }
}
...
function showLoading(isLoading) {
    ...
    if (buttonsDisabled && appState.gameState) {
        DOMElements.actionButton.classList.add('loading');
        DOMElements.actionButton.innerHTML = '<span class="btn-spinner" aria-hidden="true"></span>';
    } else {
        DOMElements.actionButton.classList.remove('loading');
...
function setAuthMode(register) {
    ...
    DOMElements.authSubmitButton.textContent = register ? '注册' : '登录';
    DOMElements.authToggleButton.textContent = register ? '已有账号？登录' : '没有账号？注册';
    DOMElements.passwordInput.autocomplete = register ? 'new-password' : 'current-password';
...
async function handleAuthSubmit(event) {
    ...
    if (!/^[A-Za-z0-9_]+$/.test(username)) {
        DOMElements.loginError.textContent = '用户名只能包含字母、数字和下划线';
        return;
    }

    DOMElements.authSubmitButton.disabled = true;
    DOMElements.loginError.textContent = '';
    ...
    try {
        ...
        // Auth cookie is set by the server; now initialize the game.
        await initializeGame();
    } catch (error) {
        DOMElements.loginError.textContent = error.message;
    } finally {
        DOMElements.authSubmitButton.disabled = false;
    }
}
...
function init() {
    ...
    DOMElements.authToggleButton.addEventListener('click', () => setAuthMode(!isRegisterMode));
